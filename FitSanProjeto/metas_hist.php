...
if (mysqli_num_rows($retorno_all) === 0) {
    ...
    $dataanterior = '';
    echo '<ul class="timeline timeline-inverse col-12">';
    while ($linha = mysqli_fetch_array($retorno_all)) {
        ...
        $dataatual = date('d/m/Y', dataParse($linha['data_final']));
        ...
        ?>     
        <li>
            <i class="fa fa-thumbs-o-up bg-yellow-gradient"></i>

            <div class="timeline-item">               
                <span class="time col-md-5 col-sm-5 col-xs-12"><?= '<b style="float: right;">Peso inicial: ' . htmlspecialchars($linha['peso_inicial']) . 'kg  |  Meta: ' . $linha['peso_final'] . 'kg</b>' ?></span>
                <h3 class="timeline-header"><strong><?= date('d M Y', dataParse(htmlspecialchars($linha['data_inicial']))) . ' - ' . date('d M Y', dataParse(htmlspecialchars($linha['data_final']))) ?></strong> <b class="label label-danger"><?php echo htmlentities($linha['tipo']) ?></b><?php
                ...
                <div class="timeline-body col-md-12 col-sm-12 col-xs-12" style="overflow-x: auto; white-space: nowrap;">
                    <div class="col-sm-7 col-md-8 chart_historico" id="<?= 'div' . $linha['id'] ?>" >                            
                        <?php
                    ...
                    <div class="col-sm-5 col-md-4 col-xs-12">
                        ...
                        <select name="<?= $linha['id'] ?>" id="<?= 'mes_hist' . $linha['id'] ?>" class="form-control mes_hist" >
                            ...
                            while ($linha_meses = mysqli_fetch_array($resultado_meses)) {
                                ?>
                                <option value="<?= htmlspecialchars($linha_meses['meses']) ?>"><?= ($anos) ? date('M - Y', dataParse(htmlspecialchars($linha_meses['data_add']))) : ucfirst(utf8_encode(strftime("%B", strtotime(htmlspecialchars($linha_meses['data_add']))))) ?></option>
                                <?php
                            }
                            ?>  
                        </select>
                        <table class="table table-striped planilha dataTable">
                            ...
                            while ($linha_dados = mysqli_fetch_array($resultado_dados)) {
                                ...
                                if ($explode[1] !== 0) {
                                    ...
                                    $peso_inicial_dif = htmlspecialchars($linha_max['peso_inicial_dif']);
                                }
                                ?>

                                <tr>
                                    <td><?= ($anos) ? date('M/Y', dataParse(htmlspecialchars($linha_dados['max_data_add']))) : date('M', dataParse(htmlspecialchars($linha_dados['max_data_add']))) ?></td>
