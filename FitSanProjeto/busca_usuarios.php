...
if(tipoLogado('profissional')){
    $query = "select * from usuario u left join vinculo v on v.aluno_id = u.id and v.profissional_id = $_SESSION[id] where u.tipo_id=1 and u.status = 'ativado'";
    //$sql = "select * from vinculo where profissional_id=$_SESSION[id]";
    $usuario_busca = 'aluno_id'; 
}else{
    $query = "select * from usuario u left join vinculo v on v.profissional_id = u.id and v.aluno_id = $_SESSION[id] where u.tipo_id=2 and u.status = 'ativado'";
